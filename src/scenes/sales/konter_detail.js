...
export default class History extends Component {
  ...
  async componentDidMount() {
    ...
    await GET('riwayat/transaksi/'+this.props.detail.user.id+'/'+date, {}, true).then(async (result) => {
      var total = 0
      var success = 0
      result.forEach(item => {
        if(item.status.toLowerCase() == 'success'){
          total = parseInt(total) + parseInt(item.grand_total)
          success = success + 1
        }
      })
  ...
  async setDate(newDate) {
	...
	render() {
      ...
      <View style={styles.container}>
        ...
        <HeaderBack title={"Riwayat Transaksi "+this.props.detail.user.name} />
