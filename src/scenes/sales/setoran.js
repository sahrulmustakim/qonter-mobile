...
export default class Setoran extends Component {
  ...
  async componentDidMount() {
    ...
    // Setoran History
    await GET('sales/setoran/'+this.state.userdata.id, {}, true).then((item) => {
      console.log(JSON.stringify(item))
      this.setState({ setoran: item.setoran })
      this.setState({ history: item.history })
    })

    // Setoran Paid
    await GET('sales/setoran/history/'+this.state.userdata.id, {}, true).then(async (result) => {
  ...
  changeFoto(){
	...
	render() {
      ...
      <View style={styles.container}>
        ...
        <HeaderBack title={'Setoran Saya'} />
        
        <ScrollView style={styles.container} scrollEnabled={true}>
          <View style={styles.container_form}>
            <Label style={[styles.labelForm, {textAlign: "center"}]}>Jumlah Setoran yang harus dibayar</Label>
            <Label style={[styles.titleBigBlack, {textAlign: "center", paddingBottom: 15}]}>Rp{(this.state.setoran != null && this.state.setoran.length > 0) ? formatPrice(this.state.setoran[0].amount) : 0}</Label>
            ...
            {
              (this.state.setoran != null && this.state.setoran.length > 0 && this.state.setoran[0].payment == null && this.state.loading == false) ? 
                ...
                <Form style={{marginBottom: 12}}>
                  ...
                  <View style={{alignItems: "center", width: '100%', paddingLeft: 15, paddingRight: 15}}>
                    <Row>
                      ...
                      {
                        (this.state.setoran != null && this.state.setoran.length > 0) ? 
                        <Button style={{width: '100%', borderRadius: 5}} block danger onPress={() => this.upload(this.state.setoran[0].id)}>
            ...
            {
              (this.state.setoran != null && this.state.setoran.length > 0 && this.state.setoran[0].amount > 0) ?
              <Grid>
                <Row style={{borderBottomWidth: 1, borderBottomColor: '#F1F3F6', marginBottom: 5, marginTop: 15}}>
                  <Col>
                    <Text style={[styles.titleBoldGreen, {paddingBottom: 10}]}>Detail Setoran</Text>
                  </Col>
                </Row>
                { 
                  this.state.history.map((trx, trxkey) => {
                    return (
                      <Row key={trxkey} style={{borderBottomWidth: 1, borderBottomColor: '#F1F3F6', marginBottom: 10, paddingBottom: 10}}>
                        ...
                        <Col size={10} style={{justifyContent: "center"}}>
                          <Grid>
                            <Col>
                              <Text style={[styles.titleSmallBlack]}>{(trx.type == 'topup') ? 'Topup Saldoku' : 'Pembayaran Tagihan'} - {trx.konter_name}</Text>
                            </Col>
                          </Grid>
                          <Text style={[styles.title]}>Rp{formatPrice(trx.amount)}</Text>
                        </Col>
                        <Col size={4} style={{justifyContent: "center", alignItems: "flex-end"}}>
                          <Text style={[styles.titleSmallBlack, {marginBottom: 5}]}>{Moment(trx.created_at).format('DD MMMM YYYY')}</Text>
                          <Text style={{backgroundColor: 'red', color: '#FFF', borderRadius: 5, paddingLeft: 8, paddingRight: 8}}>
                            {'Belum Setor'}
                          </Text>
                        </Col>
                      </Row>
            ...
            <Grid>
              <Row style={{borderBottomWidth: 1, borderBottomColor: '#F1F3F6', marginBottom: 5, marginTop: 15}}>
                <Col>
                  <Text style={[styles.titleBoldGreen, {paddingBottom: 10}]}>Riwayat Setoran Saya</Text>
                </Col>
              </Row>
              { 
