...
export default class History extends Component {
  ...
  async componentDidMount() {
    ...
    // PRODUCTS
    await GET('list/konter/'+this.state.userdata.id, {}, true).then(async (result) => {
      await this.setState({ 
        products: [{
          title: 'Daftar Pengajuan Konter',
          values: result
  ...
  _onRefresh = () => {
	...
	render() {
      ...
      <View style={styles.container}>
        ...
        <HeaderBack title="Persetujuan Qonter" />
                ...
                return (
                  <Grid key={key}>
                    ...
                    { item.values.map((unit, key2) => {
                      return (
                        <TouchableOpacity key={key2} onPress={() => Actions.sales_approval_detail({ detail: unit })}>
                          <Row style={{borderBottomWidth: 1, borderBottomColor: '#F1F3F6', marginBottom: 10, paddingBottom: 10}}>
                              ...
                              <Row style={{paddingTop: 5}}>
                                {
                                  ...
                                  <Col>
                                    <Text style={[styles.titleBoldOrange]}>Rp{formatPrice(unit.request[0].amount)}</Text>
                                  </Col>
                                  :false
                                }
                              </Row>
                            ...
                            {
                              (typeof unit.request !== 'undefined' && unit.request.length > 0) ? 
                                <Col size={6} style={{justifyContent: "center", alignItems: "flex-end"}}>
                                  <Text style={[styles.titleSmallBlack, {marginBottom: 5}]}>{Moment(unit.request[0].created_at).format('DD MMMM YYYY hh:mm')}</Text>
                                  <Text style={[(unit.request[0].status.toLowerCase() == 'true') ? styles.titleBoldGreen : (unit.request[0].status.toLowerCase() == 'false') ? styles.titleBoldRed : styles.titleBoldOrange, {textAlign: 'right'}]}>{(unit.request[0].status.toLowerCase() == 'true') ? 'Aktif' : (unit.request[0].status.toLowerCase() == 'false') ? 'Tidak Disetujui' : 'Menunggu Konfirmasi' }</Text>
                                </Col>
                              : 
                                <Col size={6} style={{justifyContent: "center", alignItems: "flex-end"}}></Col>
                            }
